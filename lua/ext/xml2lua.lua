...
local XmlParser = require("ext.XmlParser")
...
function xml2lua.parser(handler)
    if handler == xml2lua then
        error(
            "You must call xml2lua.parse(handler) instead of xml2lua:parse(handler)")
    end
...
local function getFirstKey(tb)
    if type(tb) == "table" then
        for k, _ in pairs(tb) do return k end
        return nil
    end
...
local function parseTableKeyToXml(xmltb, tagName, fieldValue, level)
    ...
    table.insert(xmltb,
                 spaces .. '<' .. tagName .. attrsStr .. '>' .. strValue .. '</' ..
                     tagName .. '>')
end
...
function xml2lua.toXml(tb, tableName, level)
    ...
    local xmltb =
        (tableName ~= '' and level == 1) and {'<' .. tableName .. '>'} or {}
    ...
    for k, v in pairs(tb) do
        if type(v) == 'table' then
            ...
            if type(k) == 'number' then k = tableName end
            parseTableKeyToXml(xmltb, k, v, level)
        end
    end
